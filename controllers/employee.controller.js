...
router.get('/', async (req, res) => {
       const employee = await service.getAllEmployee()
       res.send(employee)
 })
   
router.get('/:id', async (req, res) => {
    const employee = await service.getEmployeeById(req.param.id)
...
router.delete('/:id', async (req, res) => {
    const affectedRows = await service.deleteEmployee(req.param.id)
    if (affectedRows  == 0)
        res.status(404).json('no record with given id :' + req.params.id)
    else
